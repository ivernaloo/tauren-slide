...
//            console.info("params.length :",params.length)
    ...
    $.ui.Carousel = function(ctrls ,contents ,css ,options){
        ...
        t.ctrls = ctrls,
            ...
            t.current;
            t.btn.prev && t.btn.prev.addEvent("mouseover",function(event){
                t.pause(event)
            })
            t.btn.prev && t.btn.prev.addEvent("mouseout",function(event){
                t.cycle(event)
            })
            t.btn.next && t.btn.next.addEvent("mouseover",function(event){
                t.pause(event)
            })
            t.btn.next && t.btn.next.addEvent("mouseout",function(event){
                t.cycle(event)
            })
            ctrls.addEvent("mouseover",function(event){
                t.pause(event)
            })
            ctrls.addEvent("mouseout",function(event){
                t.cycle(event)
            })
            contents.addEvent("mouseover",function(event){
                t.pause(event)
                                })
            contents.addEvent("mouseout",function(event){
                t.cycle(event)
            })
        /*
         * @检查参数
    ...
    $.ui.Carousel.prototype = {
        ...
        slide: function(type, next){
             ...
             * */
            if( t.step == 0 || t.step == null ){
                t.step = t.stepMap["x"]($active);
                if (t.step == 0 || t.step == null) {
                    t.step = 310;
